...
class Ctl_Manager_Yuyue extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        if($SO = $this->GP('SO')){
            $pager['SO'] = $SO;
            if($SO['manager_id']){$filter['manager_id'] = $SO['manager_id'];}
        ...
        if($items = K::M('manager/yuyue')->items($filter, null, $page, $limit, $count)){
        ...
        $this->tmpl = 'admin:manager/yuyue/items.html';
    }

    public function so()
    {
        $this->tmpl = 'admin:manager/yuyue/so.html';
    }
        ...
        if($this->checksubmit()){
            if(!$data = $this->GP('data')){
                ...
                if($yuyue_id = K::M('manager/yuyue')->create($data)){
                    $this->err->add('添加内容成功');
                    $this->err->set_data('forward', '?manager/yuyue-index.html');
                }
            } 
        }else{
           $this->tmpl = 'admin:manager/yuyue/create.html';
        }
    }
    ...
    public function edit($yuyue_id=null)
    {
        if(!($yuyue_id = (int)$yuyue_id) && !($yuyue_id = $this->GP('yuyue_id'))){
        ...
        }else if(!$detail = K::M('manager/yuyue')->detail($yuyue_id)){
            ...
            if(!$data = $this->GP('data')){
                ...
                $data['is_read'] = 1;
                if(K::M('manager/yuyue')->update($yuyue_id, $data)){
                    $this->err->add('修改内容成功');
                }  
            } 
        }else{
        	...
        	$this->tmpl = 'admin:manager/yuyue/edit.html';
        }
    }
    ...
    public function delete($yuyue_id=null)
    {
        if($yuyue_id = (int)$yuyue_id){
            if(K::M('manager/yuyue')->delete($yuyue_id)){
                $this->err->add('删除成功');
            }
        }else if($ids = $this->GP('yuyue_id')){
            if(K::M('manager/yuyue')->delete($ids)){
                $this->err->add('批量删除成功');
            }
        }else{
